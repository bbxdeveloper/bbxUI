...
export class CommonService {
  ...
  HandleError(err: any, preMessage: string = ""): void {
    ...
    if (!!err?.error?.errors && err.error.errors.constructor === Object) {
      ...
      this.toastrService.show(preMessage + errorMsgs, Constants.TITLE_ERROR, Constants.TOASTR_ERROR);
      ...
      if (msg.length > 0 || errors.length > 0) {
        this.toastrService.show(preMessage + msg + errors, Constants.TITLE_ERROR, Constants.TOASTR_ERROR);
      } else {
        this.toastrService.show(preMessage + err.message, Constants.TITLE_ERROR, Constants.TOASTR_ERROR);
      }
    }
  }
}
